...
{	
	...
	switch(viewMode)
	{
		...
		{
			gaCxWin = screen.width;
			gaCyWin = screen.height;
			break;
		}
	...
	screenRect = CGRectMake(0, 0, screen.width, screen.height);
	return YES;
}
...
{
	int plusX = 0, plusY = 0;
	float scaleX = mainView->viewScaleX;
	float scaleY = mainView->viewScaleY;
	...
	view.frame = CGRectMake(ho->hoX+plusX, ho->hoY+plusY, width, height);
	view.transform = CGAffineTransformMakeScale(1/scaleX, 1/scaleY);
}
...
-(void)touchesBegan:(NSSet *)touchesA withEvent:(UIEvent *)event
{
	...
	if (!bFlag)
	{
		[self mouseDown:YES];
		if(!isOverSubapp)
			[self mouseClicked:firstTouchTapCount];
	}
	...
	if(run != nil && run->nSubApps > 0)
	{
		for(int i=0; i<run->nSubApps; ++i)
		{
			CRunApp* subRunApp = run->subApps[i]->subApp;
...
-(NSString*)getRelativePath:(NSString*)path
{
	...
	int appPathLength = [appEditorPathname length];

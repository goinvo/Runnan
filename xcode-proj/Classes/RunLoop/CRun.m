...
-(void)initAsmLoop
{
	if (rh3Panic == 0)
	{
		[rhApp->spriteGen winSetColMode:CM_BITMAP];				// Collisions precises
		[self f_ObjMem_Init];
	}
}
-(void)f_ObjMem_Init
			...
			if ((oiPtr->oiFlags & OIF_GLOBAL) != 0)
			{
				...
				while (YES)
				{
					...
					else
					{
						...
						saveValues->pValues = (CValue**)malloc(VALUES_NUMBEROF_ALTERABLE*sizeof(CValue*));
...
-(void)scrollLevel
{
	...
	// Clear sprites
	[rhApp->spriteGen activeSprite:nil withFlags:AS_REDRAW andRect:CRectNil()];		// AS_DEACTIVATE
	...
	[self f_UpdateWindowPos:rhFrame->leX withY:rhFrame->leY];
	...
	else
	{
		...
		{
			if ((rhFrame->layers[0]->dwOptions & (FLOPT_REDRAW)) != 0)
				[self redrawLevel:DLF_DONTUPDATE | DLF_DONTUPDATECOLMASK];
			else
				[self redrawLevel:DLF_DONTUPDATE | DLF_DONTUPDATECOLMASK | DLF_SKIPLAYER0];
		}
	}
}
...
{	
	...
	[rhApp->spriteGen spriteClear];
	[rhApp->spriteGen spriteUpdate];
	[rhApp->spriteGen spriteDraw:rhApp->renderer];
...
-(void)redrawLevel:(int)flags
{
	...
	BOOL		bTotalColMask = ((rhFrame->leFlags & LEF_TOTALCOLMASK) != 0);
	...
	if (flags & DLF_DRAWOBJECTS)
	{
		...
		if ( (flags & DLF_REDRAWLAYER) != 0 )
		{
			...
			[rhApp->spriteGen activeSprite:nil withFlags:AS_REDRAW andRect:CRectNil()];
		}
	...
	for ( ; nLayer < rhFrame->nLayers; nLayer++)
	{
		...
		*/
		...
		if ( (pLayer->dwOptions & (FLOPT_XCOEF | FLOPT_YCOEF)) != 0 )
		{
			if ( (pLayer->dwOptions & FLOPT_XCOEF) != 0 )
				dxLayer = (int)((float)dxLayer * pLayer->xCoef);
			if ( (pLayer->dwOptions & FLOPT_YCOEF) != 0 )
...
//		plo = [rhFrame->LOList getLOFromIndex:pLayer->nFirstLOIndex];
		...
		if ( (pLayer->dwOptions & FLOPT_TOHIDE) == 0 || nLayer == 0 )
		{
			...
			for (i=0; i<nLOs; i++)
			{
				...
				do
				{
					...
					{
						[self y_Ladder_Add:nLayer withX1:rc.left andY1:rc.top andX2:rc.right andY2:rc.bottom];
						...
						)
					{
						...
						{
							if ( cm_box!=0 )
								[rhFrame->colMask fillRectangle:rc.left withY1:rc.top andX2:rc.right-1 andY2:rc.bottom-1 andValue:obst];
							else
							{
								if ( pMask == nil )
								{
								...
								if ( pMask == NULL )
									...
									[rhFrame->colMask orMask:pMask withX:rc.left andY:rc.top andPlane:CM_OBSTACLE|CM_PLATFORM andValue:obst];
							}
						}
						...
						if ( v == OBSTACLE_PLATFORM )
						{
							...
							if ( cm_box!=0 )
							{
								[rhFrame->colMask fillRectangle:rc.left withY1:rc.top andX2:rc.right-1 andY2:(min((int)(rc.top+HEIGHT_PLATFORM),(int)rc.bottom)-1) andValue:CM_PLATFORM];
							}
							else
							{
								if ( pMask == NULL )
								...
								if ( pMask == NULL )
									...
									[rhFrame->colMask orPlatformMask:pMask withX:rc.left andY:rc.top];
							}
						}
						...
						if ( nLayer > 0 || !bLayer0Invisible)
						{
							...
							if ( *ppSpr == nil )
							{
								switch ( typeObj ) {
									...
									case OBJ_BOX:
										*ppSpr = [rhApp->spriteGen addOwnerDrawSprite:rc.left withY1:rc.top andX2:rc.right andY2:rc.bottom andLayer:plo->loLayer andZOrder:i*4+nCurSprite andBackColor:0 andFlags:(dwFlags | SF_COLBOX) andObject:nil andDrawable:poc];
										break;
										
										// Backdrop: sprite
									case OBJ_BKD:
										*ppSpr = [rhApp->spriteGen addSprite:rc.left withY:rc.top andImage:((COCBackground*)poc)->ocImage andLayer:plo->loLayer andZOrder:i*4+nCurSprite andBackColor:0 andFlags:dwFlags andObject:hoPtr];
										[rhApp->spriteGen modifSpriteEffect:*ppSpr withInkEffect:poi->oiInkEffect andInkEffectParam:poi->oiInkEffectParam];
										break;
									...
									default:
										if ( hoPtr != NULL )
										{
											*ppSpr = [rhApp->spriteGen addOwnerDrawSprite:rc.left withY1:rc.top andX2:rc.right andY2:rc.bottom andLayer:plo->loLayer andZOrder:i*4*nCurSprite andBackColor:0 andFlags:(dwFlags | SF_COLBOX | SF_NOKILLDATA) andObject:hoPtr andDrawable:hoPtr];
										}
										break;
								}
							}
								...
								switch ( typeObj )
								{
									...
									case OBJ_BOX:
									{
										...
										if ( rc.left != rcSpr.left || rc.top != rcSpr.top || rc.right != rcSpr.right || rc.bottom != rcSpr.bottom )
											[rhApp->spriteGen modifOwnerDrawSprite:*ppSpr withX1:rc.left andY1:rc.top andX2:rc.right andY2:rc.bottom];
									...
									break;
										...
										// Backdrop: sprite
									case OBJ_BKD:
										[rhApp->spriteGen modifSprite:*ppSpr withX:rc.left andY:rc.top andImage:((COCBackground*)poc)->ocImage];
				...
				if ( bOut )
				{
					...
					{
						[rhApp->spriteGen delSprite:*ppSpr];
						*ppSpr = nil;
					}
				}
	...
	if ( bTotalColMask )
	{
		...
		[rhFrame->colMask setOrigin:dxLayer withDY:dyLayer];
	}
}
		...
		if (noMove != 0)
		{
			...
			else
			{
				...
				if (nLayer < rhFrame->nLayers)
				{
					...
					if ((pHo->hoOEFlags & OEFLAG_MOVEMENTS) == 0)
					{
						pHo->hoX = nX;
						pHo->hoY = nY;
					}
	...
	for (nObject = 0; nObject < rhNObjects; nObject++)
	{
		...
		if (nLayer == hoPtr->hoLayer || nLayer == LAYER_ALL)
		{
			if (hoPtr->ros != nil)
			{
				if (hoPtr->roc->rcSprite != nil)
				{
					[rhApp->spriteGen activeSprite:hoPtr->roc->rcSprite withFlags:AS_REDRAW andRect:CRectNil()];
				}
...
-(CArrayList*)getLayerZones:(int)nLayer
{
	...
	if ( pZones == nil && (rhFrame->leWidth >= OBJZONE_WIDTH * 2 || rhFrame->leHeight >= OBJZONE_HEIGHT * 2) )
	{
		...
		for (int i=0; i<nLOs; i++, plo1 = [rhFrame->LOList getLOFromIndex:pLayer->nFirstLOIndex+i])
		{
			...
			int minzy = 0;
			if ( rc.top >= 0 )
				minzy = min(rc.top / OBJZONE_HEIGHT + 1, nyz-1);
			int maxzy = 0;
			if ( rc.bottom >= 0 )
				maxzy = min(rc.bottom / OBJZONE_HEIGHT + 1, nyz-1);
			for (int zy=minzy; zy<=maxzy; zy++)
			{
				int minzx = 0;
				if ( rc.left >= 0 )
					minzx = min(rc.left / OBJZONE_WIDTH + 1, nxz-1);
				int maxzx = 0;
				if ( rc.right >= 0 )
					maxzx = min(rc.right / OBJZONE_WIDTH + 1, nxz-1);
				for (int zx=minzx; zx<=maxzx; zx++)
				{
					// Add object to zone list
					int z = zy * nxz + zx;
					CArrayList* pZone = [pZones get:z];
					if ( pZone == nil )
					{
						//ASSERT(z < nz);
					...
					[pZone add:(void*)(i+pLayer->nFirstLOIndex)];
				}
			}
		}
	}
			...
			if (pbkd->x == toadd->x && pbkd->y == toadd->y && pbkd->nLayer == toadd->nLayer && pbkd->img == toadd->img && (pbkd->inkEffect & EFFECT_MASK) == EFFECT_NONE)
			{
				...
				if (pbkd->inkEffect != toadd->inkEffect || pbkd->inkEffectParam != toadd->inkEffectParam)
				{
					...
					for (int j = 0; j < 4; j++)
					{
						if (pbkd->pSpr[j] != nil)
						{
							[rhApp->spriteGen modifSpriteEffect:pbkd->pSpr[j] withInkEffect:pbkd->inkEffect andInkEffectParam:pbkd->inkEffectParam];
						}
					}
				}
				[toadd release];
	...
	if ((pLayer->dwOptions & (FLOPT_TOHIDE | FLOPT_VISIBLE)) == FLOPT_VISIBLE)
	{
		...
		do
		{
			...
			if (rc.left < x2edit + COLMASK_XMARGIN + 32 && rc.top < y2edit + COLMASK_YMARGIN)
			{
				...
				if (v == OBSTACLE_LADDER)
				{
					[self y_Ladder_Add:pbkd->nLayer withX1:rc.left andY1:rc.top andX2:rc.right andY2:rc.bottom];
				}
				...
				if (rc.left <= x2edit && rc.top <= y2edit && rc.right >= 0 && rc.bottom >= 0)
				{
					...
					pbkd->pSpr[nCurSprite] = [rhApp->spriteGen addSprite:rc.left withY:rc.top andImage:img andLayer:pbkd->nLayer andZOrder:0x10000000 + nIdx * 4 + nCurSprite andBackColor:0 andFlags:dwFlags andObject:nil];
					// voir si fixer variable interne a pbkd
					[rhApp->spriteGen modifSpriteEffect:pbkd->pSpr[nCurSprite] withInkEffect:pbkd->inkEffect andInkEffectParam:pbkd->inkEffectParam];
				}
                ...
                if (toadd->nLayer == 0 && v != OBSTACLE_TRANSPARENT)
                {
                    ...
                    if (flgColMaskEmpty == NO)
                    {
                        ...
                        {
                            [rhFrame->colMask fillRectangle:rc.left withY1:rc.top andX2:rc.right andY2:rc.bottom andValue:obst];
                        }
                        else
                        {
                            mask = [[rhApp->imageBank getImageFromHandle:img] getMask:GCMF_OBSTACLE withAngle:0 andScaleX:1.0 andScaleY:1.0];
                            [rhFrame->colMask orMask:mask withX:rc.left andY:rc.top andPlane:CM_OBSTACLE | CM_PLATFORM andValue:obst];
                        }
                    }
                    ...
                    if (v == OBSTACLE_PLATFORM)
                    {
                        ...
                        if (toadd->colMode==CM_BOX)
                        {
                            [rhFrame->colMask fillRectangle:rc.left withY1:rc.top andX2:rc.right andY2:min((int)(rc.top + HEIGHT_PLATFORM), (int)rc.bottom) andValue:CM_PLATFORM];
                        }
                        else
                        {
                            mask = [[rhApp->imageBank getImageFromHandle:img] getMask:GCMF_OBSTACLE withAngle:0 andScaleX:1.0 andScaleY:1.0];
                            [rhFrame->colMask orPlatformMask:mask withX:rc.left andY:rc.top];
                        }
                    }				
				}
			}
		...
		for (i = 0; i < [pLayer->pBkd2 size]; i++)
		{
			...
			if (pbkd->nLayer == nLayer)		// Heu ... c'est la peine de faire ce test?
			{
				...
				do
				{
					...
					if (pMask != nil)
					{
						if ([pMask testPoint:x - rc.left withY:y - rc.top])
						{
							bFound = YES;
							break;
						}
	...
	if ((pLayer->dwOptions & FLOPT_TOHIDE) == 0)
	{
		...
		for (i = 0; i < [pLayer->pBkd2 size]; i++)
		{
			...
			if (rc.left <= x2edit && rc.top <= y2edit && rc.right >= 0 && rc.bottom >= 0)
			{
				...
				if (pbkd->pSpr[nCurSprite] == nil)
				{
					pbkd->pSpr[nCurSprite] = [rhApp->spriteGen addSprite:screenPosX withY:screenPosY andImage:img andLayer:pbkd->nLayer andZOrder:0x10000000 + i * 4 + nCurSprite andBackColor:0 andFlags:dwFlags andObject:nil];
				...
				{
					[rhApp->spriteGen modifSprite:pbkd->pSpr[nCurSprite] withX:screenPosX andY:screenPosY andImage:img];
				}
			}
...
{ 
		...
		rhQuit == LOOPEXIT_NEWGAME)
	{
		if (rhApp->parentApp==nil)
		{
			...
			rhApp->oldFrameImage = [[CRenderToTexture alloc] initWithWidth:rhApp->renderer->currentWidth andHeight:rhApp->renderer->currentHeight andRunApp:rhApp];
...
-(CArrayList*)objectAllCol_IXY:(CObject*)pHo withImage:(short) newImg andAngle:(int)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andX:(int)newX andY:(int)newY andColList:(short*)pOiColList
{    
    CArrayList* list=nil;
...
-(CArrayList*)objectAllCol_IXY:(CObject*)pHo withImage:(short) newImg andAngle:(int)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andX:(int)newX andY:(int)newY
{
	...
	if ((pHo->hoFlags & (HOF_REALSPRITE | HOF_OWNERDRAW)) != 0)
	{
		...
		{
			list = [rhApp->spriteGen spriteCol_TestSprite_All:pHo->roc->rcSprite withImage:newImg andX:newX - rhWindowX andY:newY - rhWindowY andAngle:newAngle andScaleX:newScaleX andScaleY:newScaleY andFlags:0];
		}
...
// ----------------------------------------------
-(int)colMask_TestObject_IXY:(CObject*)pHo withImage:(short)newImg andAngle:(int)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andX:(int)newX andY:(int)newY andFoot:(int)htfoot andPlane:(int)plan
{
	int res = 0;
...
-(BOOL)getMouseOnObjectsEDX:(short)oiList withNegation:(BOOL)nega
{
	...
	CSprite* curSpr = [rhApp->spriteGen spriteCol_TestPoint:nil withLayer:LAYER_ALL andX:x andY:y andFlags:0];
	CObject* pHoFound;
	while (curSpr != nil)
	{
		...
		curSpr = [rhApp->spriteGen spriteCol_TestPoint:curSpr withLayer:LAYER_ALL andX:x andY:y andFlags:0];
	}
...
// -----------------------------------------------------------------------
+(void)setXPosition:(CObject*)hoPtr withX:(int)x
{
	if (hoPtr->rom != nil)
	{
...
+(void)setYPosition:(CObject*)hoPtr withY:(int)y
{
	if (hoPtr->rom != nil)
	{
...
{
	CExtStorage* e = [self getStorage:id];
	if (e == nil)
	{
		if (rhApp->extensionStorage == nil)
		{

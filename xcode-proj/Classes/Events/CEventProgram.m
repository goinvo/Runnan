							...
							case PARAM_FLAG_EXP:
								...
								while (expPtr->expCode.expLCode.expCode!=0)
								{
                                    ...
                                    if (bAllocateStrings && expPtr->expCode.expLCode.expCode==((3<<16)|65535))
                                    {
                                        ...
                                        {
                                            int l=strlen((char*)&expPtr->expu.expw.expWParam0);
			...
			if (oilPtr->oilOEFlags&OEFLAG_MOVEMENTS)
			{
				...
				if ( ss!=0 )
				{
					...
					while (*ulPtr!=0xFFFFFFFF)
					{
						...
						{
							if (evtPtr->evtCode.evtLCode.evtCode==( ACTL_EXTWRAP | (((long)oilPtr->oilType)&0xFFFF) ))
							{
								fWrap|=d;
							}
						}
        ...
        for (oil = 0; oil < rhPtr->rhMaxOI; oil++)
        {
            ...
            if ((o & 0x8000) == 0)
            {
                do
                {
                    ...
                    if ((hoPtr->hoOEFlags & OEFLAG_SPRITES) != 0 && (hoPtr->hoLimitFlags & OILIMITFLAGS_QUICKCOL) == 0)
                    {
...
-(CObject*)get_ParamActionObjects:(short)qoil withAction:(LPEVT)evtPtr
{
	...
	if (pObject != nil)
	{
		...
		{
			// Pas de suivant
			evtPtr->evtFlags &= ~ACTFLAGS_REPEAT;		// Ne pas refaire cette action
			return pObject;
		}
		else
		{
...
-(void)call_Actions:(LPEVT)pActions
{
	...
	if ( pEvg->evgFlags & EVGFLAGS_LIMITED )   				// Les actions limitees et validees
	{
		...
		{
			WORD w_cx = (WORD)rhPtr->rhLoopCount;
			WORD w_dx = pEvg->evgInhibit;
		...
		{
			if ( (pEvg->evgInhibitCpt & 0xFFFF) != 0 )		// sur?
				pEvg->evgInhibitCpt--;
			else
				return;
		}
		...
		if ( (pEvg->evgFlags & EVGFLAGS_NOMORE) != 0 )
		{
			...
			if ( dwmax != 0 &&		// Premiere fois?
				...
				return;
			pEvg->evgInhibitCpt = (WORD)(dwt+pEvg->evgInhibit);		// Plus timer possible= timer maxi
		}
	}
...
-(void)endShuffle
{
	...
	int x2=pHo2->hoX;
	int y2=pHo2->hoY;
	[CRun setXPosition:pHo1 withX:x2];
	[CRun setYPosition:pHo1 withY:y2];
	[CRun setXPosition:pHo2 withX:x1];
	[CRun setYPosition:pHo2 withY:y1];
...
-(void)onMouseButton:(int)nClicks
{
	...
	while (YES)
	{
		spr = [rhPtr->rhApp->spriteGen spriteCol_TestPoint:spr withLayer:LAYER_ALL andX:mx andY:my andFlags:0];
		if (spr == nil)
		{
			break;
		}
	...
	for (int nObjects = 0; nObjects < rhPtr->rhNObjects; nObjects++)
	{
		...
		{
			int x = pHo->hoX - pHo->hoImgXSpot-rhPtr->rhWindowX;
			if (rhPtr->rh2MouseX <= mx && (x + pHo->hoImgWidth > rhPtr->rh2MouseX))
			{
				int y = pHo->hoY - pHo->hoImgYSpot-rhPtr->rhWindowY;

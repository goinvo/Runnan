...
-(CApproach)mpApproachSprite:(int)destX withDestY:(int)destY andMaxX:(int)maxX andMaxY:(int)maxY andFoot:(short)htFoot andPlane:(short)planCol
{
	...
	do
	{
		if ([self tst_SpritePosition:x + hoPtr->hoAdRunHeader->rhWindowX withY:y + hoPtr->hoAdRunHeader->rhWindowY andFoot:htFoot andPlane:planCol andFlag:NO])
		{
			// On peut y aller
			loinX = x;
			loinY = y;
			...
			if (x == oldX && y == oldY)
			{
				...
				{
					if ([self tst_SpritePosition:presX + hoPtr->hoAdRunHeader->rhWindowX withY:presY + hoPtr->hoAdRunHeader->rhWindowY andFoot:htFoot andPlane:planCol andFlag:NO])
					{
						x = presX;
						y = presY;
					}
		...
		else
		{
			...
			if (x == oldX && y == oldY)
			{
				if (loinX != presX || loinY != presY)
				{
					if ([self tst_SpritePosition:loinX + hoPtr->hoAdRunHeader->rhWindowX withY:loinY + hoPtr->hoAdRunHeader->rhWindowY andFoot:htFoot andPlane:planCol andFlag:NO])
					{
						ret.isFound = YES;
						ret.point.x = loinX;
						ret.point.y = loinY;

	...
	switch (MP_Type)
	{
		...
		case 0:         // MPTYPE_WALK:
			...
			if ([self tst_SpritePosition:hoPtr->hoX withY:hoPtr->hoY + 10 andFoot:(short) MP_HTFOOT andPlane:CM_TEST_PLATFORM andFlag:YES] == NO)
			{
				...
				hoPtr->hoX = ap.point.x + hoPtr->hoAdRunHeader->rhWindowX;
				hoPtr->hoY = ap.point.y + hoPtr->hoAdRunHeader->rhWindowY;
				MP_NoJump = NO;
			}
			else
			{
...
-(void)stop
{
	...
	switch (hoPtr->hoAdRunHeader->rhEvtProg->rhCurCode & 0xFFFF0000)
	{
		...
		case (-14 << 16):	    // CNDL_EXTCOLLISION:
		{
			...
			{
				CApproach ap = [self mpApproachSprite:scrX withDestY:scrY andMaxX:hoPtr->roc->rcOldX - hoPtr->hoAdRunHeader->rhWindowX andMaxY:hoPtr->roc->rcOldY - hoPtr->hoAdRunHeader->rhWindowY andFoot:(short)MP_HTFOOT andPlane:CM_TEST_PLATFORM];
			...
			if (MP_Type == MPTYPE_WALK)
			{
				...
				ap = [self mpApproachSprite:scrX withDestY:scrY andMaxX:hoPtr->roc->rcOldX - hoPtr->hoAdRunHeader->rhWindowX andMaxY:hoPtr->roc->rcOldY - hoPtr->hoAdRunHeader->rhWindowY andFoot:(short)0 andPlane:CM_TEST_PLATFORM];
				if (ap.isFound)
				{
					hoPtr->hoX = ap.point.x + hoPtr->hoAdRunHeader->rhWindowX;
					hoPtr->hoY = ap.point.y + hoPtr->hoAdRunHeader->rhWindowY;
			...
			{
				// On essaye de positionner le sprite contre l'obstacle
				CApproach ap = [self mpApproachSprite:scrX withDestY:scrY andMaxX:hoPtr->roc->rcOldX - hoPtr->hoAdRunHeader->rhWindowX andMaxY:hoPtr->roc->rcOldY - hoPtr->hoAdRunHeader->rhWindowY andFoot:(short)0 andPlane:CM_TEST_PLATFORM];
				if (ap.isFound)
				{
					hoPtr->hoX = ap.point.x + hoPtr->hoAdRunHeader->rhWindowX;
					hoPtr->hoY = ap.point.y + hoPtr->hoAdRunHeader->rhWindowY;
...
-(void)calcMBFoot
{
	...
	{
		MP_XMB = -hoPtr->hoAdRunHeader->rhWindowX;							//&; X ecran du point milieu bas (sous hot spot)
		MP_YMB = hoPtr->hoImgHeight - hoPtr->hoAdRunHeader->rhWindowY - hoPtr->hoImgYSpot;	//; Y ecran du point milieu bas	

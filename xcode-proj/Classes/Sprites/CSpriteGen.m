...
-(CSprite*)modifSpriteEx:(CSprite*)ptSpr withX:(int)xSpr andY:(int)ySpr andImage:(short)iSpr andScaleX:(float)fScaleX andScaleY:(float)fScaleY andScaleFlag:(BOOL)bResample andAngle:(int)nAngle andRotateFlag:(BOOL)bAntiA
{
	// Pointer sur le sprite et son image
	if (ptSpr == nil)
		return ptSpr;
...
-(void)setSpriteAngle:(CSprite*)ptSpr withAngle:(int)nAngle andFlag:(BOOL)bAntiA
{
	if (ptSpr != nil)
	{
		BOOL bOldAntiA = ((ptSpr->sprFlags & SF_ROTATE_ANTIA) != 0);
...
-(CMask*)getSpriteMask:(CSprite*)ptSpr withImage:(short)newImg andFlags:(int)nFlags andAngle:(int)newAngle andScaleX:(double)newScaleX andScaleY:(double)newScaleY
{
	if (ptSpr != nil)
	{
...
-(void)pasteSpriteEffect:(CRenderer*)renderer withImage:(short)iNum andX:(int)iX andY:(int)iY andFlags:(int)flags andInkEffect:(int)effect andInkEffectParam:(int)effectParam
{
	...
	// Blit
	[renderer renderImage:(ITexture*)ptei withX:x1 andY:y1 andWidth:ptei->width andHeight:ptei->height andInkEffect:effect andInkEffectParam:effectParam];
}
		...
		if (bEvenNoCol || (ptSpr->sprFlags & SF_RAMBO) != 0)
		{
			if (xp >= ptSpr->sprX1 && xp < ptSpr->sprX2 && yp >= ptSpr->sprY1 && yp < ptSpr->sprY2)
			{
				...
				if(pMask != nil)
				{
					...
					Point2D position = {ptSpr->sprX, ptSpr->sprY};
					Point2D hotspot = {info.xSpot, info.ySpot};
					Scale2D scale = {ptSpr->sprScaleX, ptSpr->sprScaleY};
					Mat3f worldToMask = WorldspaceToMaskspace(position, hotspot, scale, ptSpr->sprAngle);
					Vec3f maskCoord = MatrixMult(worldToMask, MakeVec3f(xp, yp, 1));
...
//--------------------------------------------------------------------------------------//
-(CArrayList*)spriteCol_TestSprite_All:(CSprite*)ptSpr withImage:(short)newImg andX:(int)newX andY:(int)newY andAngle:(int)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andFlags:(int)dwFlags
{
	int nLayer;
	int cm = colMode;
	CArrayList* list = nil;
...
//
-(CSprite*)spriteCol_TestSprite:(CSprite*)ptSpr withImage:(short)newImg andX:(int)newX andY:(int)newY andAngle:(int)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andFoot:(int)subHt andFlags:(int)dwFlags
{
	int nLayer;

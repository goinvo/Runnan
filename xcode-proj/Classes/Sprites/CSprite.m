...
-(void)draw:(CRenderer*)renderer
{
	...
	if(toDraw != nil)
	{
		...
		[renderer renderScaledRotatedImage:(ITexture*)toDraw withAngle:sprAngle andScaleX:sprScaleX andScaleY:sprScaleY andHotSpotX:hsx andHotSpotY:hsy andX:sprX andY:sprY andWidth:toDraw->width andHeight:toDraw->height andInkEffect:sprEffect andInkEffectParam:sprEffectParam];
	}
}
...
-(void)calcBoundingBox:(short)newImg withX:(int)newX andY:(int)newY andAngle:(int)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andRect:(CRect)prc
{
	CImage* ptei;
